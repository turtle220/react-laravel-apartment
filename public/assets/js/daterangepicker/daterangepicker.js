    ...
    DateRangePicker.prototype = {
        ...
        updateView: function () {
            if (this.timePicker) {
                ...
                if (!this.endDate) {
                    this.container
                        .find('.calendar-time.right select')
        ...
        updateCalendars: function () {
            ...
            if (this.clickedDate) {
                ...
                if (typeof this.fetchEvents == 'function' && this.setEvents) {
                    ...
                    if (this.singleDatePicker) {
                        this.fetchEvents(
                            this.leftCalendar.calendar[0][0],
                            this.leftCalendar.calendar[5][6],
                            this,
                            this.setEvents
                        );
                    } else {
                        this.fetchEvents(
                            this.leftCalendar.calendar[0][0],
                            this.rightCalendar.calendar[5][6],
                            this,
                            this.setEvents
                        );
                    }
        ...
        renderCalendar: function (side) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    if (this.minimumCheckin > 0) {
                        for (var _i = 0; _i < this.minimumCheckin; _i++) {
                            var _today = new Date(
                                new Date().getTime() + _i * 24 * 60 * 60 * 1000
                            );
                    ...
                    if (this.disabledDates != null) {
                        var currentDate = calendar[row][col].format(
                            'YYYY-MM-DD'
                        );
                    ...
                    for (i = 0; i < this.allEvents.length; i++) {
                        ...
                        if (
                            ...
                            if (this.showEventTooltip) {
                                ...
                                event_html =
                                    '<div class="event-tooltip-wrap"><div class="' +
                                    event_class +
                                    ' event event-' +
                                    row +
                                    '-' +
                                    col +
                                    '" data-date-group="' +
                                    this.allEvents[i].end +
                                    '">' +
                                    this.allEvents[i].event +
                                    '</div>';
                                if (
                                    typeof this.allEvents[i].has_starttime !==
                                    'undefined'
                                ) {
                            ...
                            if (
                                typeof this.allEvents[i].status == 'string' &&
                                this.allEvents[i].status == 'not_available'
                            ) {
                                if (typeof this.classNotAvailable == 'object') {
                                    $.each(this.classNotAvailable, function (
                                        index,
                                        val
                                    ) {
                                        classes.push(val);
            ...
            if (this.showTodayButton && side == 'left') {
                html +=
                    '<a href="javascript: void(0);" class="button button-default btn btn-success btn-small btn-today">' +
                    this.locale.today +
                    '</a>';
            }
            if (side == 'left') {
                this.container
                    .find('.calendar.left .calendar-table')
                    .html(html);
            }

            if (!this.singleDatePicker && side == 'right') {
                this.container
                    .find('.calendar.right .calendar-table')
                    .html(html);
            }
        },
        ...
        renderTimePicker: function (side) {
            ...
            if (side == 'left') {
                ...
                var timeSelector = this.container.find(
                    '.calendar-time.right div'
                );
                var currentTime = timeSelector
                    .find('.timepicker option:selected')
                    .val();
            ...
            if (side === 'left') {
                html =
                    '<label>' +
                    this.locale.labelStartTime +
                    '<select class="select-dropdown timepicker hourselect">';
            } else {
                html =
                    '<label>' +
                    this.locale.labelEndTime +
                    '<select class="select-dropdown timepicker hourselect">';
            }
            ...
            for (var i = start; i <= end; i++) {
                ...
                var disabled = false;
                if (minDate && time.hour(i).isBefore(minDate)) disabled = true;
                if (maxDate && time.hour(i).isAfter(maxDate)) disabled = true;
        ...
        move: function () {
            ...
            if (!this.parentEl.is('body')) {
                parentOffset = {
                    ...
                    left:
                        this.element.offset().left - this.parentEl.scrollLeft(),
                };
            }
        ...
        respontosingle: function (e) {
            ...
            if (
                $(window).width() - parent.offset().left < this.widthCalendar &&
                parent.offset().left < this.widthCalendar
            ) {
            ...
            if (
                $(window).width() - parent.offset().left < _width_single &&
                parent.offset().left >= _width_single
            ) {
                this.container
                    .removeClass(
                        'opensleft opensright moveleft moveright openscenter'
                    )
                    .addClass('moveleft opensleft');
                this.opens = 'left';
            } else {
                this.opens = 'right';
                this.container
                    .removeClass(
                        'moveleft opensleft opensright moveright openscenter'
                    )
                    .addClass('moveright opensright');
            }
            if (this.alwaysShow) {
                ...
                if (!this.showCalendar) {
                    if (this.sameDate) {
                        if ($(e.target).parent().hasClass('start-group')) {
                            var end_date = $(e.target)
                                .parent()
                                .find('.event')
                                .attr('data-date-group');
                            end_date = moment(end_date, this.locale.format);
        ...
        elementChanged: function () {
            ...
            if (
                moment(end.format(this.locale.format)).isSame(
                    moment(start.format(this.locale.format))
                ) &&
                !this.singleDay &&
                !this.sameDate
            ) {
