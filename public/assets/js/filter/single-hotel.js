(function ($) {
    ...
    $(document).on(
        'click',
        '.sts-pag .page-numbers a.page-numbers:not(.current, .dots)',
        function (e) {
            e.preventDefault();
            var t = $(this);
        ...
        xhr = $.ajax({
            ...
            success: function (doc) {
                if (doc.status) {
                    if (
                        data['layout'] === 'grid' &&
                        divResult.hasClass('list')
                    ) {
                        divResult.removeClass('list');
                    }
                    if (
                        data['layout'] === 'list' &&
                        divResult.hasClass('grid')
                    ) {
            ...
            complete: function () {
                ...
                divResult.find('img').one('load', function () {
                    if (
                        divResult.find('img.loaded').length ===
                        divResult.find('img').length
                    ) {
            ...
            if (sURLVariables.length) {
                for (var i = 0; i < sURLVariables.length; i++) {
                    ...
                    if (sParameterName.length) {
                        ...
                        res[decodeURIComponent(sParameterName[0])] =
                            val == 'undefined' ? '' : val;
                    }
                }
            }
